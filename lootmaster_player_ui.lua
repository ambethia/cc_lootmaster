...
function LootMaster:CreateLootSelectFrame()
    ...
    local btnNeed = CreateFrame("Button", nil, frame, "UIPanelButtonTemplate")
	btnNeed:SetScript("OnClick", function()
            self:SaveNote(frame)
            self:SendItemWanted( frame.data.lootmaster, frame.data.id, self.RESPONSE.NEED, frame.data.note );
            self:RemoveLoot( frame.data.id );
            self:UpdateLootUI();
    end)
	btnNeed:SetPoint("BOTTOMLEFT",icon,"BOTTOMRIGHT",10,0)
	btnNeed:SetHeight(25)
	btnNeed:SetWidth(140)
	btnNeed:SetText("Need / Mainspec")
    
    local btnGreed = CreateFrame("Button", nil, frame, "UIPanelButtonTemplate")
	btnGreed:SetScript("OnClick", function()
            self:SaveNote(frame)
            self:SendItemWanted( frame.data.lootmaster, frame.data.id, self.RESPONSE.GREED, frame.data.note )
            self:RemoveLoot( frame.data.id );
            self:UpdateLootUI();
    end)
	btnGreed:SetPoint("BOTTOMLEFT",btnNeed,"BOTTOMRIGHT",10,0)
	btnGreed:SetHeight(25)
	btnGreed:SetWidth(140)
	btnGreed:SetText("Greed / Offspec")
	...
	btnPass:SetPoint("BOTTOMLEFT",btnGreed,"BOTTOMRIGHT",10,0)
	btnPass:SetHeight(25)
	btnPass:SetWidth(100)
    ...
    b:SetScript("OnUpdate", function( o, elapsed )
        ...
        frame.lblTimeout:SetText( format("%s secs untill pass", ceil(frame.data.timeoutLeft)) )
